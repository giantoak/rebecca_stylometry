...
if __name__ == '__main__':
    ...
    path = '/Users/cusgadmin/BackPageStylometry/data-drop-1/extractions/groundTruth/FindMatchings/src/'
    all_names = []
...
"""
    ...
    '''
    mainDir = '/Users/cusgadmin/BackPageStylometry/data-drop-1/extractions'
    noChunkDir = 'data_noChunk/emailAndphone_noRepeats_G2Posts_L=100'
    files = open(mainDir+'/APTP_L=100_3.txt')
    for row in files:
        rowAsArray = row.split(" ")
        folder1 = rowAsArray[0] + "_diff";
        file1 = rowAsArray[1] + "_title.txt";
        #file1 = rowAsArray[1] + ".txt";
        folder2 = rowAsArray[2] + "_diff";
        file2 = rowAsArray[3].rstrip() + "_title.txt";
        #file2 = rowAsArray[3].rstrip() + ".txt";
        
        ad1 = open(mainDir + '/' + noChunkDir + '/' + folder1 + '/' + file1,'r')
        ad_data = ad1.read()
        if "backpage.com" in ad_data: ad_data = ad_data.replace('backpage.com','')
        if "escorts -" in ad_data: ad_data = ad_data.replace('escorts -','')
        if "body rubs -" in ad_data: ad_data = ad_data.replace('body rubs -','')
        if "strippers and strip clubs -" in ad_data: ad_data = ad_data.replace('strippers and strip clubs -','')
        if "domination and BDSM services -" in ad_data: ad_data = ad_data.replace('domination and BDSM services -','')
        if "transsexual escorts -" in ad_data: ad_data = ad_data.replace('transsexual escorts -','')
        if "Adult web site directory -" in ad_data: ad_data = ad_data.replace('male escorts -','')
        if "adult jobs -" in ad_data: ad_data = ad_data.replace('adult jobs -','')
        ad1.close()
        #print ad_data
        
        ad2 = open(mainDir + '/' + noChunkDir + '/' + folder2 + '/' + file2,'r')
        ad_data2 = ad2.read()
        if "backpage.com" in ad_data2: ad_data2 = ad_data2.replace('backpage.com','')
        if "escorts -" in ad_data2: ad_data2 = ad_data2.replace('escorts -','')
        if "body rubs -" in ad_data2: ad_data2 = ad_data2.replace('body rubs -','')
        if "strippers and strip clubs -" in ad_data2: ad_data2 = ad_data2.replace('strippers and strip clubs -','')
        if "domination and BDSM services -" in ad_data2: ad_data2 = ad_data2.replace('domination and BDSM services -','')
        if "transsexual escorts -" in ad_data2: ad_data2 = ad_data2.replace('transsexual escorts -','')
        if "Adult web site directory -" in ad_data2: ad_data2 = ad_data2.replace('male escorts -','')
        if "adult jobs -" in ad_data2: ad_data2 = ad_data2.replace('adult jobs -','')
        #print ad_data2
        ...
        v2 = Values(ad_data2, path, all_names)
        f = Features(v1, v2)
        print f.extract_features(True)
        #print f.cosine_similarity(v1.extract_breaklines(),v2.extract_breaklines())

    files = open(mainDir + '/RAS_L=100_3.txt')
    for row in files:
        folder = mainDir + '/' + noChunkDir + '/' + row.rstrip() + "_same/"
        ...
        for i in range(0, len(listOfFiles)):
            a = listOfFiles[i]
            if 'tagged' in a : continue
            elif 'title' in a and found==False:
            #elif 'title' not in a and found==False :
                file1 = a
                found = True
            elif 'title' in a and found==True: file2 = a
            #elif 'title' not in a and found==True : file2 = a
        
        ad1 = open(folder + file1,'r')
        ad_data = ad1.read()
        if "backpage.com" in ad_data: ad_data = ad_data.replace('backpage.com','')
        if "escorts -" in ad_data: ad_data = ad_data.replace('escorts -','')
        if "body rubs -" in ad_data: ad_data = ad_data.replace('body rubs -','')
